  ...
  <nav class="nav-primary container py-2 px-4 lg:px-16">
    <div class="flex flex-wrap lg:flex-nowrap justify-between items-center mx-auto">
      <a class="brand flex items-center" href="{{ home_url('/') }}">
        {{-- {!! $siteName !!} --}}
        @if (! is_page('pros-mulher'))
          <img class="w-20" alt="Logo do partido PROS Amapá" src="@asset('images/logo.png')" />
        @else
          <img class="w-20" alt="Logo do partido PROS Amapá" src="@asset('images/logoprosmulher.jpg')" />
        @endif
      </a>
      ...
      <div class="hidden justify-between items-center w-full lg:flex lg:w-auto lg:order-1" id="mobile-menu-3">
        ...
        {!! wp_nav_menu(['theme_location' => 'primary_navigation', 'menu_class' => 'flex flex-col mt-4 lg:flex-row lg:mt-0 lg:text-sm lg:font-medium', 'echo' => false]) !!}
      </div>
    </div>
  </nav>
</header>
